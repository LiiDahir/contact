...
class Contact:
    ...
    def write_data(self,):
        data={'contact':self.numbers,'user':self.user}
        data_file = self.get_path()
        ...
        for name in names:
            ...
            for index in range(0,len(name)-2):
                if name[index] == name[index+1] == name[index+2]:
                    return False
                if name[index].lower() in valows and name[index+1].lower() in valows and name[index+2].lower() in valows :
                    return False
    ...
    def edit(self):
        ...
        if id in self.numbers:
            ...
            self.check_status()
    def promt(self,promt,datatype,func,hubi= True):
        stat = True
        info = ''
        while stat:
            try:
                if datatype == 'int':
                    info = int(input(promt))
                if datatype == 'str':
                    info = input(promt)
                if datatype == 'float':
                    info = float(input(promt)) 
            except:
                stat = True
            if func(info):
                stat = False 
            if hubi == True:
                promt = promt.replace(':',"")+"correctly : "
                hubi = False
        return info
    
    def is_int(self, promt,num,hubi= True):
        stat = True
        while stat:
            try:
                info = int(input(promt))
    ...
    def check_status(self,num=0):
        ...
        if status==1:
            ...
            print("4. Search")
            print("5. Exit ")
            select = self.is_int("select of theme : ",5)
            if select == 1:
                self.add()
            elif select == 2:
                self.edit()
